...
export const MusicHistory = () => {
  ...
  return (
    <Box>
      {loading && <Box height={250}><Center h={'100%'}><CircularProgress isIndeterminate/></Center></Box>}
      {!loading && (
        <Box p={3} borderWidth="1px" borderRadius="lg">
          <VStack
            ...
            align="stretch"
          >
            <Text fontSize="3xl" fontWeight={'bold'}>{"What I've been listening to"}</Text>
            {tracks.slice(0, track_limit).map((it, i) => (
              <Grid templateColumns="repeat(10, 1fr)" gap={3} key={i} w={'100%'} borderRadius={5} as={'a'}
                    href={it.url}>
                <Text gridColumn={'span 4'}>{it.name}</Text>
                <Box gridColumn={'span 3'}>{it.artist['#text']}</Box>
                <Text gridColumn={'span 3'} fontSize={'sm'} color={secondaryText}>
                  {!it.date && "right now!"}
                  {it.date && <Tooltip label={it.date['#text']}>
                    {"about " + moment.unix(it.date.uts).fromNow()}
                  </Tooltip>}
                </Text>
              </Grid>
            ))}
